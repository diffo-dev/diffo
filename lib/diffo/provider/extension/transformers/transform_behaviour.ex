...
defmodule Diffo.Provider.Extension.Transformers.TransformBehaviour do
      ...
      if spec[:id] do
        before_body =
          quote do
            ...
            |> Diffo.Provider.Extension.Feature.set_features_argument(features())
            |> Diffo.Provider.Extension.Characteristic.set_characteristics_argument(
              characteristics()
            )
  ...
  defp inject_create_arguments(dsl_state) do
    action_create_declarations =
      Transformer.get_entities(dsl_state, [:provider, :behaviour, :actions])
      |> Enum.filter(&is_struct(&1, ActionCreate))

    Enum.reduce(action_create_declarations, dsl_state, fn %ActionCreate{name: action_name},
                                                          dsl_state ->
      action =
