...
defmodule Diffo.Provider.Extension.Verifiers.VerifySpecification do
  @moduledoc "Verifies specification DSL values satisfy the Specification resource constraints"
  ...
  defp check_id(dsl_state, resource) do
    spec_id = Verifier.get_option(dsl_state, [:provider, :specification], :id)

    if spec_id && !Diffo.Uuid.uuid4?(spec_id) do
      [
        DslError.exception(
          module: resource,
          path: [:provider, :specification, :id],
  ...
  defp check_attributes(dsl_state, resource) do
    ...
    Enum.flat_map(@spec_fields, fn field ->
      value = Verifier.get_option(dsl_state, [:provider, :specification], field)
        ...
        case Ash.Type.apply_constraints(attr.type, value, attr.constraints) do
          ...
          {:error, errors} ->
            [
              DslError.exception(
                module: resource,
                path: [:provider, :specification, field],
