...
defmodule Diffo.Provider.Extension.Verifiers.VerifyBehaviour do
  ...
  alias Diffo.Provider.Extension.ActionUpdate

  @impl true
  def verify(dsl_state) do
    resource = Verifier.get_persisted(dsl_state, :module)
    behaviour_actions = Verifier.get_entities(dsl_state, [:provider, :behaviour, :actions])
        ...
        %ActionCreate{name: name} ->
          if MapSet.member?(create_names, name) do
            ...
            [
              DslError.exception(
                module: resource,
                path: [:provider, :behaviour, :actions],
                message:
