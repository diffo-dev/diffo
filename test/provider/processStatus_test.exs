defmodule Diffo.Provider.ProcessStatus.ProcessStatus do
  ...
  describe "Diffo.Provider.ProcessStatus create" do
    test "create a process status - success" do
      ...
      process_status = Diffo.Provider.ProcessStatus.create!(%{instance_id: instance.id, code: "NBNACC-1003", severity: :warning, message: "nbnProductOrder cancelled"})
    ...
    test "create a process status with a parameterised message - success" do
      ...
      process_status = Diffo.Provider.ProcessStatus.create!(%{instance_id: instance.id, code: "NBNACC-1003", severity: :warning, message: "nbnProductOrder cancelled", parameterized_message: parameterized_message})
      assert process_status.code == "NBNACC-1003"
    end
  end
  ...
  describe "Diffo.Provider.ProcessStatus encode" do
    test "encode json - success" do
      ...
      assert encoding == "{\"code\":\"NBNACC-1003\",\"severity\":\"warning\",\"message\":\"nbnProductOrder cancelled\",\"timestamp\":\"now\"}"
    end
    ...
    test "encode json with parameterized message - success" do
      ...
      process_status = Diffo.Provider.ProcessStatus.create!(%{instance_id: instance.id, code: "NBNACC-1003", severity: :warning, message: "nbnProductOrder cancelled", parameterized_message: parameterized_message})
      encoding = Jason.encode!(process_status) |> Diffo.Util.summarise_dates()
      assert encoding == "{\"code\":\"NBNACC-1003\",\"severity\":\"warning\",\"message\":\"nbnProductOrder cancelled\",\"parameterizedMessage\":{\"reason\":\"cancelled due to force majeure\"},\"timestamp\":\"now\"}"
    end
  end
end
